...
import com.thanos.common.crypto.jce.ECKeyAgreement;
import com.thanos.common.crypto.jce.ECKeyFactory;
import com.thanos.common.crypto.jce.ECKeyPairGenerator;
import com.thanos.common.crypto.jce.ECSignatureFactory;
 ...
 */
@Deprecated
public class ECKeyOld extends SecureKey implements Serializable {

    private static final Logger logger = LoggerFactory.getLogger(ECKeyOld.class);
    ...
    public static final ECKeyOld DUMMY;
     ...
     */
    public ECKeyOld(Provider provider, SecureRandom secureRandom) {
        ...
        this.publicKey = pub.getEncoded(/* compressed */ false);
    }
     ...
     * @param secureRandom -
     */
    public ECKeyOld(SecureRandom secureRandom) {
        this(CastleProvider.getSpongyInstance(), secureRandom);
    }
     ...
     * All private key operations will use the provider.
     */
    public ECKeyOld(Provider provider, PrivateKey privKey, ECPoint pub) {
        this.provider = provider;
     ...
     * BouncyCastle will be used as the Java Security Provider
     */
    public ECKeyOld(BigInteger priv, ECPoint pub) {
        this(
     ...
     * @return -
     */
    public static ECKeyOld fromPrivate(BigInteger privKey) {
        return new ECKeyOld(privKey, CURVE.getG().multiply(privKey));
    }
     ...
     * @return -
     */
    public static ECKeyOld fromPrivate(byte[] privKeyBytes) {
     ...
     * @return -
     */
    public static ECKeyOld fromPrivateAndPrecalculatedPublic(BigInteger priv, ECPoint pub) {
        return new ECKeyOld(priv, pub);
    }
     ...
     * @return -
     */
    public static ECKeyOld fromPrivateAndPrecalculatedPublic(byte[] priv, byte[] pub) {
        check(priv != null, "Private key must not be null");
        check(pub != null, "Public key must not be null");
        return new ECKeyOld(new BigInteger(1, priv), CURVE.getCurve().decodePoint(pub));
    }
     ...
     * @return -
     */
    public static ECKeyOld fromPublicOnly(ECPoint pub) {
        return new ECKeyOld(null, pub);
    }
     ...
     * @return -
     */
    public static ECKeyOld fromPublicOnly(byte[] pub) {
        return new ECKeyOld(null, CURVE.getCurve().decodePoint(pub));
    }
     ...
     * @deprecated per-point compression property will be removed in Bouncy Castle
     */
    public ECKeyOld decompress() {
        if (!pub.isCompressed())
            return this;
        else
            return new ECKeyOld(this.provider, this.privKey, decompressPoint(pub));
    }

    /**
     * @deprecated per-point compression property will be removed in Bouncy Castle
     */
    public ECKeyOld compress() {
        if (pub.isCompressed())
            return this;
        else
            return new ECKeyOld(this.provider, this.privKey, compressPoint(pub));
    }
     ...
     */
    public static ECKeyOld fromNodeId(byte[] nodeId) {
        ...
        return ECKeyOld.fromPublicOnly(pubBytes);
    }
    ...
    public ECDSASignature signECDSA(byte[] messageHash) {
        ...
        for (int i = 0; i < 4; i++) {
            byte[] k = ECKeyOld.recoverPubBytesFromSignature(i, sig, messageHash);
    ...
    public static byte[] signatureToKeyBytes(byte[] messageHash, ECDSASignature sig) throws SignatureException {
        ...
        int recId = header - 27;
        byte[] key = ECKeyOld.recoverPubBytesFromSignature(recId, sig, messageHash);
        if (key == null)
     ...
     * @return ECKey
     */
    public static ECKeyOld signatureToKey(byte[] messageHash, String signatureBase64) throws SignatureException {
        final byte[] keyBytes = signatureToKeyBytes(messageHash, signatureBase64);
        return ECKeyOld.fromPublicOnly(keyBytes);
    }
     ...
     * @return ECKey
     */
    public static ECKeyOld signatureToKey(byte[] messageHash, ECDSASignature sig) throws SignatureException {
    ...
    public boolean verify(byte[] data, byte[] signature) {
        return ECKeyOld.verify(data, signature, doGetPubKey());
    }



    /**
     * Verifies the given R/S pair (signature) against a getHash using the public key.
     *
     * @param sigHash   -
     * @param signature -
     * @return -
    ...
    public boolean verify(byte[] sigHash, ECDSASignature signature) {
        return ECKeyOld.verify(sigHash, signature, doGetPubKey());
    }
     ...
     * @return ECKey
     */
    public static ECKeyOld recoverFromSignature(int recId, ECDSASignature sig, byte[] messageHash) {
        final byte[] pubBytes = recoverPubBytesFromSignature(recId, sig, messageHash);
        if (pubBytes == null) {
            return null;
        } else {
    ...
    public boolean equals(Object o) {
        if (this == o) return true;
        if (o == null || !(o instanceof ECKeyOld)) return false;

        ECKeyOld ecKey = (ECKeyOld) o;
    ...
    public static void main(String[] args) {
        ECKeyOld ecKey = ECKeyOld.fromPrivate(Hex.decode("77ee7a385f1e653966908d580eba32a090ddadf862633c329d5c73dfdfb54f6f"));
        String msg = "hello";
